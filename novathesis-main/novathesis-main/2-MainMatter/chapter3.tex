...
coordinates its perturbation across agents and divides it across time. It is broken down
into four questions: whether dividing the attack across time buys more damage per
attacked step (RQ1), whether coordinating the perturbation across agents increases the
damage (RQ2), which architectural choices of the victim determine its vulnerability
(RQ3), and whether the fraction of routing decisions an attack changes is a meaningful
measure of the damage it does (RQ4).
...
work~\cite{amaral2021maddpg-routing}, and the FGSM baseline was measured in the same
...
\begin{table}[htbp]
  \centering
  \small
  \caption{Research questions and the comparisons that answer them. The measures are
  ...
  \begin{tabular}{>{\raggedright\arraybackslash}p{1.6cm}
    ...
    RQ4     & Routing decisions changed against delivery lost
...
them only through their observations and actions. The FGSM baseline additionally
differentiates the agents' actors with respect to their inputs; the learned adversary of
this thesis never does.
...
work~\cite{TODO-colleague-fgsm}. It is a gradient-based attack, implemented
...
sigmoid of that path's bottleneck utilisation, read from the observation, so that
ascending it steers the policy towards congested paths rather than inducing a
misclassification. For the GNN variants the gradient is taken through the encoder, with
...
which an attack is judged. This thesis retains all four.
...
its two mechanisms: the SAJA-inspired coordination and the PGD-inspired timing described
...
that the baseline was too weak.
...
\item \textbf{(B) Strategically-timed, critical-state attack, inspired by PGD:} the attack
is divided across time, as PGD divides its budget into smaller
steps~\cite{madry2019deeplearningmodelsresistant}. A budget limits how often the
adversary may act, and a critical-state score decides when it does, so that the attack
is spent at the moments when the network's redundancy is thinnest.
\end{itemize}
...
several redirected flows converge on one shared link, and these lie outside the
...
objectives of the form of Equation~\eqref{eq:separable_objective}. How it is trained is
described in Section~\ref{subsec:learned_adversary}.
...
Limiting how often an attack acts was first studied by Lin et
...
(Section~\ref{subsec:lit_timed_attacks}). The timing mechanism of this thesis shares
that idea but differs from their attack in three respects. First, its critical-state
...
Equation~\eqref{eq:joint_attack}. Lemma~1 takes the perturbed state itself as the
...
perturbation, and the critic is additionally conditioned on the available bandwidth of
every link together with each agent's most common queued destination. This yields one
transition per step for the whole group.
...
a measure of damage would conflate the two. Whether they diverge in practice is RQ4.
...
as a failure of the trained adversary or of its objective, not as evidence of
robustness. Similarly, the claim that the learned adversary is theoretically stronger
than FGSM concerns the optimal adversary; the trained adversaries only approximate it,
...
\begin{itemize}
  ...
  \item Four timing budgets between $0.10$ and $0.25$ are evaluated, together with no
        budget, and every adversary is trained at $b = 0.25$ only.
  ...
  \item The coordination and timing mechanisms are realised with a learned adversary.
        Optimisation-based PGD and SAJA attacks are not implemented, and are left for
        future work (Section~\ref{subsec:future_optimisation}).
\end{itemize}
...
adversary, extended with SAJA-inspired coordination and PGD-inspired timing, is trained
