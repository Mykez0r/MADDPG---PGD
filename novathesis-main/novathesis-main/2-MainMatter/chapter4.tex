...
record why the timing gate fired, and the chapter closes with the comparison against
FGSM and the effect of the domain clamp.
...
The FGSM-parity runs are the first in which the timing gate recorded why it acted. For every
...
moderate ($c_t < 0.75$). Figure~\ref{fig:trigger_reasons_parity}, in
Appendix~\ref{app:triggers}, shows each trigger's share of the attack arm's attacked
steps. The random control passes through the same gate, and its composition is nearly
...
The timing gate behaves as it does in the independent scope
(Figure~\ref{fig:trigger_reasons_coordinated_parity}, in Appendix~\ref{app:triggers}).
No attacked step scored $1.00$ or more, or less than $0.75$. Steps scoring between
$0.90$ and $1.00$ fire in a nearly constant number per variant at every budget, so their
share of the attacks falls from $42$--$78\%$ at $b = 0.10$ to $17$--$32\%$ at
$b = 0.25$. Ties admitted by rationing make up $22$--$58\%$ of the attacks at
$b = 0.10$ and $44$--$66\%$ at $b = 0.15$; admitting every tie would have raised the
attack rate to between $17.5\%$ and $21.3\%$ at both budgets. At $b = 0.25$ the
threshold lies between $0.82$ and $0.90$, above-threshold steps in the high-congestion
band make up $58$--$77\%$ of the attacks, and ties only $3$--$11\%$. The gate's score
depends only on link utilisation, and the close match with
Figure~\ref{fig:trigger_reasons_parity} indicates that the composition of the triggers
is set by the network and the budget rather than by how the perturbation is chosen.
...
This outcome bears directly on the robustness of the controller. The learned adversary
...
the victim, ends up doing about as much damage as a single gradient step supports the
reading that the controller is robust to observation-space attacks of this size, rather
than the reading that the FGSM baseline was simply too weak to find its vulnerabilities.
As with every result in this chapter, the conclusion concerns the adversaries that were
trained, not a bound on the damage an optimal attack could do.
