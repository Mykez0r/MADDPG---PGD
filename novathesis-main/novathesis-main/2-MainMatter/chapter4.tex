...
\begin{table}[htbp]
  \centering
  \caption[Independent scope, FGSM-parity clamp: adversarial gap by timing budget]{%
           Independent scope, FGSM-parity domain clamp. Adversarial gap in percentage
           ...
           $\varepsilon$-ball; negative values indicate the reverse. The reference is
           this study's random control, which draws each feature uniformly from that
           ball, so these gaps are not directly comparable with the FGSM gaps of
           Table~\ref{tab:fgsm_baseline}, which are measured against a different
           control.}
...
\begin{table}[htbp]
  \centering
  \caption[Independent scope, FGSM-parity clamp: ungated gaps and flip rates]{%
           Independent scope, FGSM-parity domain clamp. Ungated results at
           ...
           the attack and by the random control. The gap is measured against this
           study's uniform random control, not against the $\pm\varepsilon$ control of
           the FGSM study, so it is not directly comparable with
           Table~\ref{tab:fgsm_baseline}.}
...
\begin{table}[htbp]
  \centering
  \caption[Coordinated scope, FGSM-parity clamp: adversarial gap by timing budget]{%
           Coordinated scope, FGSM-parity domain clamp. Adversarial gap in percentage
           points by victim variant and requested timing budget, at
           $\varepsilon = 0.30$ and $n = 30$ paired episodes. Bold entries have a 95\%
           confidence interval excluding zero. As in
           Table~\ref{tab:gap_sweep_parity}, the reference is this study's uniform
           random control, so these gaps are not directly comparable with the FGSM gaps
           of Table~\ref{tab:fgsm_baseline}.}
...
\begin{table}[htbp]
  \centering
  \caption[FGSM baseline under the FGSM-parity clamp: adversarial gap and flip rates]{%
           FGSM baseline under the FGSM-parity domain clamp, $\varepsilon = 0.30$, no
           ...
           excluding zero. That control draws $\pm\varepsilon$ with a random sign for
           every feature, whereas the control behind
           Tables~\ref{tab:gap_sweep_parity} to~\ref{tab:gap_sweep_coordinated_parity}
           draws each feature uniformly from the $\varepsilon$-ball: the two sets of
           gaps are measured against different references and must not be compared
           directly. The attacks themselves are compared in
           Tables~\ref{tab:fgsm_vs_independent} and~\ref{tab:fgsm_vs_coordinated}.
           These figures are measured on the base trained victim of each
           architecture; Section~\ref{subsec:limitations_statistics} reports how far
           they move between training runs of the victim.}
...
adversary per configuration and a single trained victim per architecture;
Section~\ref{subsec:limitations_statistics} sets out what that leaves unresolved, and
reports how far the FGSM figures themselves move when the victim is retrained. As with every result in this chapter, the conclusion concerns the
...
Table~\ref{tab:gap_sweep_independent} gives the underlying values. In this setting the
damage falls on the variants with a duelling value head. All three duelling
...
The head itself cannot be the explanation. As Section~\ref{subsec:axis_value_head}
sets out, the duelling head of these critics sums two scalar streams and therefore spans
exactly the same function class as the simple head: anything one can express, the other
can. What differs is the training trajectory, since the extra parameters change the path
the critic takes and so the weights it settles on, and with them the actor that critic
trained. A pattern that tracks the duelling head is thus a property of the particular
solutions these runs happened to learn, not of the head's expressive power. Nor does the
pattern hold outside this setting: under the coordinated scope of
Section~\ref{subsec:results_coord} and under the FGSM-parity clamp of
Section~\ref{sec:results_parity_clamp} the damage falls elsewhere, which
Section~\ref{subsec:conclusions_architecture} takes up.
