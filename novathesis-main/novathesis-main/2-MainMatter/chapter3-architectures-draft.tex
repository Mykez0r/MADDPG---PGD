...
The matrix omits the \texttt{LC-Simple-GNN} cell and is therefore not a complete
factorial. Both main effects remain observable under a local critic --- the value
head through \texttt{LC-Simple}/\texttt{LC-Duelling}, and graph encoding through
\texttt{LC-Duelling}/\texttt{LC-Duelling-GNN} --- but their interaction under a
local critic is not identifiable and is instead assumed additive. That assumption
is testable rather than merely asserted: the four central-critic cells are
complete, so the same interaction is identifiable there and can be checked for
significance before the local-critic results are interpreted. Where a single
variant is reported, \texttt{CC-Simple} serves as the reference configuration,
being the canonical MADDPG formulation with no additional architectural machinery.
% TODO: state the actual reason for the omission (e.g. compute budget) once confirmed.
